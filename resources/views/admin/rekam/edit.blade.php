                ...
                <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                        ...
                        <form class="px-10 pb-6" method="post" action="{{ route('admin.rekam.update', $rekam->id) }}" enctype="multipart/form-data">
                            ...
                            <div class="mb-4">
                                <x-input-label for="pasien" :value="__('Nama Pasien')" />
                                <x-text-input id="pasien" name="pasien" type="text" class="mt-1 block w-full" :value="old('pasien', $rekam->nama_pasien)" required autofocus autocomplete="pasien" />
                                <x-input-error class="mt-2" :messages="$errors->get('pasien')" />
                            </div>
                            <div class="mb-4">
                                <x-input-label for="dokter" :value="__('Nama Dokter')" />
                                <x-text-input id="dokter" name="dokter" type="text" class="mt-1 block w-full" :value="old('dokter', $rekam->nama_dokter)" required autofocus autocomplete="dokter" />
