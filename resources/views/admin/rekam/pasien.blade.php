        ...
        <div class="p-4 sm:ml-64">
            <div class="py-12">
                ...
                @foreach($rekams as $rekam)
                    @if($rekam->nama_pasien != $previousPasien)
                        @php
                        $previousPasien = $rekam->nama_pasien;
                        @endphp

                        <h2 class="text-4xl font-extrabold dark:text-white my-6">Pasien: {{ $rekam->nama_pasien }}</h2>
                        <div class="container mx-auto gap-8 flex flex-col sm:flex-row flex-wrap">
                            @foreach($rekams as $rekamInner)
                                @if($rekamInner->nama_pasien == $rekam->nama_pasien)
                                    <div class="w-96 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                                        ...
                                        <div class="p-5 justify-between">
                                            ...
                                            <div class="mb-3 grid grid-cols-1 gap-4">
                                                <div>
                                                    <p class="mb-1 font-normal text-sm text-gray-700 dark:text-white">Dokter : {{ $rekam->nama_dokter }}</p>
